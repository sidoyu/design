...
| 자산 | 적용 수준 |
|---|---|
| sidoyu.com | 전면 적용 (1차 소비자) |
| 개인 PWA 1종 | 적용 (토큰+일부 프리미티브) |
| Chrome 확장 1종 | 부분 (색·타이포 토큰만) |
| 웹게임 1종 | UI 크롬만, 재기획 통과 후 |
| CLI 도구들 | 해당 없음 |
...
- 간격 표준: 카드 내부 `p-5` · 리스트 아이템 `p-4` · 그리드/리스트 간격 `gap-4` · 섹션 사이 `mt-10` · 제목 아래 `mt-2` · 라벨-입력 `gap-2`.
