...
> 버전: v0.5.2 (2026-08-18) · 변경 이력은 `CHANGELOG.md`
...
### BackLink
상세(자식) 페이지 상단 뒤로가기 표준. `text-xs text-ink-faint` 문단 안 `← {부모 섹션명}` 링크(`brand-underline brand-ring`). `min-h-6 min-w-6`으로 텍스트는 text-xs를 유지하면서 터치 타깃 24×24px 구조 보장(v0.5.2). 모든 상세 페이지의 **첫 요소**로 쓰고, 다음 블록은 `mt-3`.
...
- 터치/클릭 타깃: 버튼 md 40px 기준(sm은 보조 액션 한정). 문장 밖에 홀로 서는 텍스트 링크(BackLink·목록형 링크 등)는 크기 ≥24×24px 확보(WCAG 2.2 2.5.8 AA) — BackLink는 `min-h-6 min-w-6` 내장(§6), 커스텀 단독 링크도 같은 방식으로 양축을 보장한다. 본문 문장 속 링크는 인라인 예외 — 줄 높이에 종속되므로 24px을 강제하지 않는다.
- 색 단독 정보 전달 금지(§1-4).
